...
class OllamaComponent(Component):
    def __init__(self, tree, workflow, parent=None):
        ...
        self.model = self.settings.get("model", "llama3.2")
    ...
    def connect_to_client(self):
        try:
            ...
            logger.warning(f"Ollama server not available at {self.api_host}: {e}")
            self.client = None
    ...
    def generate_content(self, contents):
        ...
        response = ollama.chat(
            ...
            images=contents[1],
            options=self.base_config,
        )
        return response

    @staticmethod
    def get_usage(response) -> dict:
        prompt_eval_count = getattr(response, "prompt_eval_count", -1)
        eval_count = getattr(response, "eval_count", -1)
        t_image = -1
        t_thinking = -1
    ...
    def build_contents(self, input):
        """Baseline processor for inputs to send to Ollama"""
        ...
        for i, item in enumerate(input):
            if isinstance(item, Result):
                ...
                if typ in ["DATA", "TEXT"]:
                    ...
                    raise NotImplementedError(f"Unsupported type {typ} for ollama: {item}")
