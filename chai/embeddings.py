...
``VectorStore`` (defined in ``chai.storage`` -- persistence is Storage's job) is a SQLite-backed store with cosine search. The workflow
...
from .storage import VectorStore  # noqa: F401 -- persistence lives with Storage
