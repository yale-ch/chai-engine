...
Gemini 2.5 takes a thinking token budget; Gemini 3+ replaces it with `thinking_level` and
(from 3.6) rejects temperature/top_p outright. These tests pin the config the component
builds for each generation without talking to the API.
"""
...
class TestGemini37(unittest.TestCase):
    ...
    def test_explicit_temperature_ignored_with_warning(self):
        with self.assertLogs("chai", level="WARNING") as logs:
            c = build_gemini({"model": "gemini-3.7-flash", "temperature": 0.2})
        self.assertIsNone(c.base_config.temperature)
        self.assertTrue(any("temperature" in m for m in logs.output))
